...
def mask_account_card(type_and_num: str) -> str:
    ...
    if find_cyrillic_letters(type_and_num):
...
def get_date(date: str) -> str:
    ...
    if len(formatted_date) < 21:
        raise ValueError("Некорректный ввод. Введите дату в формате '%Y-%m-%dT%H:%M:%S.%f'.")

    iso_date = datetime.strptime(formatted_date, "%Y-%m-%dT%H:%M:%S.%f")
